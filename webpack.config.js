...
const frontendPathBase = path.join(__dirname, './frontend');

// 导出一个配置对象，webpack在启动时会根据配置内容进行打包
module.exports = {

    // 打包的入口文件
    entry: path.join(frontendPathBase, './src/js/main.js'),

    // 打包后文件输出
    output: {
        path: path.resolve(frontendPathBase, './dist'),
        filename: 'buld.js'
    },
    ...
    plugins: [
        ...
        new htmlWebpackPlugin({
            template: path.join(frontendPathBase, './src/index.html'),
            filename: 'index.html',
            inject: 'body'
        })
